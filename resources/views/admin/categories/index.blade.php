<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto p-6 min-h-screen bg-white">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pb-5 border-b border-gray-100">
            <div>
                <h1 class="text-3xl font-extrabold text-[#0A2540] tracking-tight">Manajemen Kategori</h1>
                <p class="text-sm text-gray-400 mt-1">Tambah, ubah, atau hapus kategori tugas.</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="bg-gray-50 border border-gray-200 px-4 py-2 rounded-xl text-center min-w-[100px]">
                    <span class="text-[9px] text-gray-400 uppercase font-bold block tracking-wider leading-none mb-1">Total Kategori</span>
                    <span class="text-xl font-black text-[#0A2540]">{{ $categories->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 sticky top-6">
                    <h2 class="text-base font-bold text-[#0A2540] mb-5 uppercase tracking-widest border-b pb-3">Tambah Kategori</h2>
                    
                    <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="text-xs font-bold text-[#0A2540] uppercase tracking-widest block mb-2">Nama Kategori</label>
                        ...
                        <div>
                            <label class="text-xs font-bold text-[#0A2540] uppercase tracking-widest block mb-2">Warna</label>
                            <select name="color" class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-[#FF6B00] focus:border-[#FF6B00] font-semibold text-sm text-gray-700 bg-white">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50/40 transition-all group">
                                <td class="px-6 py-4 font-bold text-gray-700 text-sm">{{ $category->name }}</td>
                                <td class="px-6 py-4">
                                    <span class="text-[10px] font-black uppercase tracking-wider bg-gray-100 text-gray-500 px-2.5 py-1 rounded border border-gray-200">
                                        {{ $category->color ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-4">
                                        ...
                                        <form method="POST" action="{{ route('admin.categories.destroy', $category->id) }}" onsubmit="return confirm('Hapus kategori ini?')" class="inline">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="3" class="px-6 py-20 text-center text-gray-400 italic">Belum ada kategori.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div id="editCategoryModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md border border-gray-100">
            <h2 class="text-lg font-bold text-[#0A2540] mb-4 uppercase tracking-wider text-xs border-b pb-2">Edit Kategori</h2>
            <form id="editCategoryForm" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="text-xs font-bold text-[#0A2540] uppercase tracking-widest block mb-2">Warna</label>
                    <select name="color" id="editCategoryColor" class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-[#FF6B00] focus:border-[#FF6B00] font-semibold text-sm text-gray-700 bg-white">
                        <option value="">-- Pilih Warna --</option>
                        <option value="blue">Blue</option>
                        <option value="green">Green</option>
                        <option value="red">Red</option>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="button" onclick="closeEditCategory()" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2.5 rounded-xl text-xs uppercase tracking-wider transition">Batal</button>
                </div>
            </form>
        </div>
    </div>
